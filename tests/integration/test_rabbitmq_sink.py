...
class TestRabbitMQIntegration:
    ...
    async def test_sink_consumer_flow(self):
        ...
        consumer = RabbitMQConsumer(
            url=url, exchange="test", queue="test_q", routing_keys=["events"]
        )
        ...
        # Find custom event
        custom_events = [
            e for e in events if e.event_type == "custom" or e.event_type == "custom_event"
        ]
        if custom_events:
    ...
    def test_circuit_breaker(self):
        ...
        with streamll.configure(sinks=[sink]), streamll.trace("test"):
            pass
